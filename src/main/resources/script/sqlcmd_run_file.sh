...
do
  ...
  SUCCESS_LOG_PATH="$DIR/suc_log_$i.txt"
  cmd="/opt/mssql-tools/bin/sqlcmd -S $HOST,$PORT -U $USER -P $PASSWORD -d $DATABASE -i '$sqlFilePath' -r0 2>$ERROR_LOG_PATH 1>$SUCCESS_LOG_PATH"
  echo "cmd:$cmd"
  eval "$cmd"
  ...
  is_file_has_content "$SUCCESS_LOG_PATH"
  if [[ "$?" -eq 1 ]]
  then
    cat "$SUCCESS_LOG_PATH"
  fi
